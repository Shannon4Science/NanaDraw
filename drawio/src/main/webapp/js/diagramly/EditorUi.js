		...
		if (this.menus != null)
		{
			...
			this.menus.addPopupMenuEditItems = function(menu, cell, evt)
			{
				...
				else
				{
					...
					// NanaDraw integration: custom menu items in embed mode
					if (urlParams['embed'] == '1' && window.parent !== window)
					{
						var nanadrawGraph = ui.editor.graph;
						var nanadrawCells = nanadrawGraph.getSelectionCells();

						menu.addSeparator();

						menu.addItem(mxResources.get('nanadrawSaveAsset'), null, function()
						{
							try
							{
								var labels = [];

								for (var i = 0; i < nanadrawCells.length; i++)
								{
									var lbl = nanadrawGraph.getLabel(nanadrawCells[i]) || '';
								...
								for (var j = 0; j < nanadrawCells.length; j++)
								{
									cellIds.push(nanadrawCells[j].id);
								}
								...
								var doExport = function()
								{
									ui.editor.exportToCanvas(function(canvas)
									{
										...
										{
											console.error('NanaDraw saveToAssets canvas error:', e);
										}
									}, null, null, null, function(err)
									{
										console.error('NanaDraw exportToCanvas error:', err);
									}, null, false, 1, true, false);
								};

								nanadrawGraph.refresh();
								setTimeout(doExport, 100);
							}
							catch (e)
							{
								console.error('NanaDraw saveToAssets error:', e);
							}
						});

						menu.addItem(mxResources.get('nanadrawRegen'), null, function()
						{
							try
							{
								var cellInfos = [];
								...
								for (var i = 0; i < nanadrawCells.length; i++)
								{
									var cell = nanadrawCells[i];
									var lbl = nanadrawGraph.getLabel(cell) || '';
									var info = {
										id: cell.id,
										label: lbl.replace(/<[^>]*>/g, '').trim()
									};

									if (cell.value != null && typeof cell.value === 'object' && typeof cell.value.getAttribute === 'function')
									{
										var vr = cell.value.getAttribute('nanadraw_visual_repr');
										var cat = cell.value.getAttribute('nanadraw_category');
										if (vr) info.visual_repr = vr;
								...
								var doRegenExport = function()
								{
									ui.editor.exportToCanvas(function(canvas)
									{
										try
										{
											...
											window.parent.postMessage(JSON.stringify({
												event: 'nanadraw:regen',
												cells: cellInfos,
												pngBase64: pngBase64,
												count: nanadrawCells.length
											}), '*');
										}
										catch (e)
										{
								...
								nanadrawGraph.refresh();
								setTimeout(doRegenExport, 100);
							}
							catch (e)
							{
								console.error('NanaDraw regen error:', e);
							}
						});

						// Crop option for single image component
						if (nanadrawCells.length == 1)
						{
							var cropCell = nanadrawCells[0];
							var cropImageUrl = null;
							var cropIsHtmlLabel = false;
							var cropState = nanadrawGraph.view.getState(cropCell);

							if (cropState != null && nanadrawGraph.getModel().isVertex(cropCell))
							{
								var cropStyleImg = mxUtils.getValue(cropState.style, mxConstants.STYLE_IMAGE, null);
								var cropShape = mxUtils.getValue(cropState.style, mxConstants.STYLE_SHAPE, null);

								if (cropStyleImg && cropShape == 'image')
								{
									cropImageUrl = cropStyleImg;
								}
								else
								{
									var cropVal = nanadrawGraph.getModel().getValue(cropCell);
									var cropLbl = '';

									if (cropVal != null && typeof cropVal === 'object' && cropVal.getAttribute)
									{
										cropLbl = cropVal.getAttribute('label') || '';
									}
									else if (typeof cropVal === 'string')
									{
										cropLbl = cropVal;
									}
									...
									if (cropLbl)
									{
										var decoded = cropLbl.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&amp;/g, '&');
										var imgMatch = decoded.match(/src="([^"]+)"/);

										if (imgMatch)
										{
											cropImageUrl = imgMatch[1];
											cropIsHtmlLabel = true;
										}
									}
								}
							}
							...
							if (cropImageUrl)
							{
								menu.addItem(mxResources.get('nanadrawCrop'), null, function()
								{
									var existingClip = cropIsHtmlLabel ? null :
										mxUtils.getValue(cropState.style, mxConstants.STYLE_CLIP_PATH, null);

									var dlg = new CropImageDialog(ui, cropImageUrl, existingClip,
										function(clipPath, cropW, cropH)
									{
										if (cropIsHtmlLabel)
										{
											// Parse inset and crop the actual image pixels
											if (!clipPath) return;

											var insetMatch = clipPath.match(/inset\(([^)]+)\)/);
											if (!insetMatch) return;

											var tokens = insetMatch[1].replace(/round.*$/, '').trim().split(/[\s,]+/);
											var topPct = parseFloat(tokens[0]) / 100;
											var rightPct = parseFloat(tokens[1]) / 100;
											var bottomPct = parseFloat(tokens[2]) / 100;
											var leftPct = parseFloat(tokens[3]) / 100;
											...
											cropImg.onload = function()
											{
												...
												var cx = Math.round(leftPct * iw);
												var cy = Math.round(topPct * ih);
												var cw = Math.round((1 - leftPct - rightPct) * iw);
												var ch = Math.round((1 - topPct - bottomPct) * ih);
												...
												var newB64 = cvs.toDataURL('image/png').split(',')[1];
												var newSrc = 'data:image/png;base64,' + newB64;
												var newLabel = '<img src="' + newSrc +
													'" width="100%" height="100%"/>';

												var model = nanadrawGraph.getModel();
												model.beginUpdate();

												try
												{
													nanadrawGraph.cellLabelChanged(cropCell, newLabel);

													var geo = nanadrawGraph.getCellGeometry(cropCell);
													...
													if (geo != null)
													{
														...
														model.setGeometry(cropCell, geo);
													}
												}
												finally
												{
										...
										{
											applyClipPath(cropCell, clipPath, cropW, cropH, nanadrawGraph);
										}
									});
	...
	EditorUi.prototype.installMessageHandler = function(fn)
	{
		...
		mxEvent.addListener(window, 'message', mxUtils.bind(this, function(evt)
		{
			...
			if (urlParams['proto'] == 'json')
			{
				...
				else if (data.action == 'mergeCells')
				{
					var graph = this.editor.graph;
					var xml = data.xml;
					...
					if (xml != null)
					{
						...
						var node = this.editor.extractGraphModel(doc2.documentElement, true);
						...
						if (node != null)
						{
							...
							try
							{
								graph.importGraphModel(node, 0, 0);
							}
							finally
							{
								graph.getModel().endUpdate();
					...
					var embedParent = window.opener || window.parent;
					if (embedParent != null)
					{
						embedParent.postMessage(JSON.stringify({
							event: 'mergeCells',
							success: true
						}), '*');
					}
				...
				else if (data.action == 'replacePageXml')
				{
					var graph = this.editor.graph;
					var xml = data.xml;
					...
					if (xml != null)
					{
						try
						{
							...
							if (node != null && node.nodeName == 'mxGraphModel')
							{
								...
								if (this.currentPage != null)
								{
									this.currentPage.root = graph.model.root;
								}
							}
				...
				else if (data.action == 'updateCells')
				{
					var graph = this.editor.graph;
					var model = graph.getModel();
					var updates = data.cells || [];
						...
						try
						{
							...
							{
								var u = updates[i];
								var cell = model.getCell(u.id);
								if (cell == null) continue;

								if (u.value !== undefined)
								{
									graph.cellLabelChanged(cell, u.value);
								}

								if (u.style !== undefined)
								{
									model.setStyle(cell, u.style);
								}

								if (u.attrs)
								{
									var val = cell.value;
									if (val != null && typeof val === 'object' && val.nodeType === 1)
									{
										for (var key in u.attrs)
										{
											if (u.attrs.hasOwnProperty(key))
											{
												val.setAttribute(key, u.attrs[key]);
											}
										}
									}
								}
								...
								if (u.geometry)
								{
									...
									{
										geo = geo.clone();
										if (u.geometry.x !== undefined) geo.x = u.geometry.x;
										if (u.geometry.y !== undefined) geo.y = u.geometry.y;
										if (u.geometry.width !== undefined) geo.width = u.geometry.width;
										if (u.geometry.height !== undefined) geo.height = u.geometry.height;
				...
				else if (data.action == 'patchCells')
				{
					var graph = this.editor.graph;
					var model = graph.getModel();
					var updates = data.update || [];
					var removes = data.remove || [];
					var addXml = data.add;
					var removedCount = 0;
					...
					try
					{
						...
						{
							var cell = model.getCell(removes[i]);
							if (cell != null) cellsToRemove.push(cell);
						}
						...
						// 2. Update cells (value / style / geometry)
						for (var i = 0; i < updates.length; i++)
						{
							var u = updates[i];
							var cell = model.getCell(u.id);
							...
							if (u.geometry)
							{
								...
								if (geo != null)
								{
									...
									if (u.geometry.width !== undefined) geo.width = u.geometry.width;
									if (u.geometry.height !== undefined) geo.height = u.geometry.height;
									if (u.geometry.relative !== undefined) geo.relative = u.geometry.relative;
									if (u.geometry.offset)
									{
										geo.offset = new mxPoint(
											u.geometry.offset.x || 0,
											u.geometry.offset.y || 0
										);
									}
									if (u.geometry.points)
									{
										geo.points = [];
										for (var p = 0; p < u.geometry.points.length; p++)
										{
											geo.points.push(new mxPoint(
												u.geometry.points[p].x || 0,
												u.geometry.points[p].y || 0
											));
										}
									}
									model.setGeometry(cell, geo);
						...
						// 3. Add new cells via XML import
						if (addXml != null)
						{
							var doc2 = mxUtils.parseXml(addXml);
							var node = this.editor.extractGraphModel(doc2.documentElement, true);
							if (node != null)
							{
								graph.importGraphModel(node, 0, 0);
