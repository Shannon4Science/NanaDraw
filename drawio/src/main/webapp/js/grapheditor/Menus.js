...
Menus.prototype.addPopupMenuCellEditItems = function(menu, cell, evt, parent)
{
	...
	// Shows edit image action if there is an image in the style or embedded in HTML value
	var hasStyleImage = graph.getModel().isVertex(cell) && mxUtils.getValue(state.style, mxConstants.STYLE_IMAGE, null) != null;
	var hasEmbeddedImage = false;
	if (!hasStyleImage && graph.getModel().isVertex(cell))
	{
		var cellVal = graph.getModel().getValue(cell);
		var rawLbl = '';
		if (cellVal != null && typeof cellVal === 'object' && cellVal.getAttribute) rawLbl = cellVal.getAttribute('label') || '';
		else if (typeof cellVal === 'string') rawLbl = cellVal;
		if (rawLbl && (rawLbl.indexOf('<img') >= 0 || rawLbl.indexOf('data:image/') >= 0 || rawLbl.indexOf('nanadraw://img/') >= 0)) hasEmbeddedImage = true;
	}
	if (hasStyleImage || hasEmbeddedImage)
	{
		menu.addSeparator();
		if (hasStyleImage) this.addMenuItem(menu, 'image', parent, evt).firstChild.nextSibling.innerHTML = mxResources.get('editImage') + '...';
