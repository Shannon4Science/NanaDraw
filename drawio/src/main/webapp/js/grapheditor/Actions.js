...
Actions.prototype.init = function()
{
	...
	this.addAction('crop...', function()
	{
		...
		if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent()) && cell != null)
		{
	    	...
	    	var imageUrl = style[mxConstants.STYLE_IMAGE], shape = style[mxConstants.STYLE_SHAPE];
	    	
			if (imageUrl && shape == 'image')
			{
				// Standard image shape — use directly
			}
			else
			{
				// Try extracting embedded <img> from HTML value (NanaDraw components)
				var cellValue = graph.getModel().getValue(cell);
				var rawLabel = null;
				if (cellValue != null)
				{
					if (typeof cellValue === 'object' && cellValue.getAttribute)
					{
						rawLabel = cellValue.getAttribute('label') || '';
					}
				...
				if (rawLabel)
				{
					var decoded = rawLabel.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&amp;/g, '&');
					var srcMatch = decoded.match(/src="([^"]+)"/);
					if (srcMatch && (srcMatch[1].indexOf('data:image/') >= 0 ||
						srcMatch[1].indexOf('nanadraw://img/') >= 0 ||
						srcMatch[1].indexOf('http') >= 0))
					{
						imageUrl = srcMatch[1];
					}
				}
			...
			var dlg = new CropImageDialog(ui, imageUrl, style[mxConstants.STYLE_CLIP_PATH], function(clipPath, width, height)
	    	{
